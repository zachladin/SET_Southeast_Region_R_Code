    ...
    for(k in 1:length(smiUnitList)){
      ...
      for(i in 1:length(stationList)){
        ...
        pos.pinCovs<-unique(new.data[,c("State","Unit_Code","Site_Name","Plot_Name","Position_Name","variable","pos.pin","Lat","Long")])
          ...
          #save station results
          try(write.csv(pos.pin.out.2, file=paste(getwd(), "Results","Results_Bayes","Station_Results",refugeName,smiUnitName,stationName,paste(stationName,"Bayes_Pin-level","slopes.Obs.csv",sep="_"),sep="/"),row.names=FALSE))


          #save compiled Position-level slopes
          try(write.csv(position.out.2, file=paste(getwd(),"Results","Results_Bayes","Station_Results",refugeName,smiUnitName,stationName,paste(stationName,"Bayes_Position-level_slopes.Obs.csv",sep="_"),sep="/"),row.names=FALSE))

          try(write.csv(station.out.2, file=paste(getwd(), "Results","Results_Bayes","Station_Results",refugeName,smiUnitName,stationName,paste(stationName,"Bayes_Station-level","slopes.Obs.csv",sep="_"),sep="/"),row.names=FALSE))
      ...
      try(write.csv(smiUnit.pin.slopes, file=paste(getwd(), "Results","Results_Bayes","SMI_unit_Results",refugeName,smiUnitName, paste(smiUnitName,"Bayes_Pin-level","slopes.Obs.csv",sep="_"),sep="/"),row.names=FALSE))

      try(write.csv(smiUnit.positions, file=paste(getwd(), "Results","Results_Bayes","SMI_unit_Results",refugeName,smiUnitName, paste(smiUnitName,"Bayes_Position-level","slopes.Obs.csv",sep="_"),sep="/"),row.names=FALSE))
